...
function describeValues(propPropertiesPath, propDescriptor) {
    ...
    var validatorNode = validatorArray.length ? validatorArray[0].get('value').node : undefined;
    var returnedExpression = bt.isMethod(validatorNode) &&
        ...
        : bt.isArrowFunctionExpression(validatorNode)
            ? validatorNode.body
            : undefined;
    var varName = (bt.isMethod(validatorNode) || bt.isArrowFunctionExpression(validatorNode)) &&
    ...
    if (bt.isBinaryExpression(returnedExpression)) {
        ...
        switch (returnedExpression.operator) {
            case '>':
                if (bt.isUnaryExpression(returnedExpression.right) &&
                ...
                break;
            case '<':
                if (bt.isUnaryExpression(returnedExpression.left) &&
        ...
        var values = bt.isCallExpression(valuesNode) &&
            ...
            ? valuesNode.callee.object.elements.map(function (e) { return e.value; })
            : undefined;
        if (values) {
            propDescriptor.values = values;
        }
